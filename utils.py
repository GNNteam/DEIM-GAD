...
def my_load_data(dataname, path='/home/sp429zxw/GADACE.main/datasets/'):
    data_dir = path + dataname + '.bin'
    graph = load_graphs(data_dir)
    return graph[0][0]

def my_load_data_bond(dataname):
    file_path='/home/sp429zxw/GADACE.main/datasets/' + dataname + '.pt'
...
def node_degrees_tensor(graph, device=None):
    deg = graph.out_degrees().float()
